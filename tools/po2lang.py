...
parser = argparse.ArgumentParser(
    description="Converts .po translatation files to .lang Minecraft translatation files."
)
parser.add_argument(
    "--watch",
    "-w",
    action="store_true",
    help="Whether to watch for file changes in the texts folder.",
)
...
def convert_file(in_path, out_path):
    ...
    for entry in polib.pofile(in_path):
        if entry.msgid != "" and not (
            entry.msgid != "pack.description" and "BP" in out_path
        ):
            string = entry.msgstr.replace('\\"', '"')
            if "\n" in string:
                sys.exit(
                    f'\033[0;31mNew line detected in translation string "{entry.msgid}" in file "{in_path}"!\033[0m'
                )
...
def update_keys(filename):
    ...
    with open(filename, "w", encoding="utf-8") as file:
        ...
        for entry in base_entries:
            ...
            file.write(
                f"msgstr \"{lang_entries.get(entry, base_entries.get(entry, ''))}\"\n"
            )
            file.write("\n")
